...
| Fichero | Casos |
|---|---|
| `test_ws_handshake.py` | Key inválida → WS se cierra con error. Handshake correcto → mensaje `ready`. `input_mode=text` no levanta transcriber (no handshake con fake transcriber). |
...
- Dependencias nuevas: `respx`, `pytest-httpserver` (opcional), `websockets` (ya en requirements)
